...
## 2. Switching Regulator (3.3V and 2.8V) Selection
...
| ![](L4971.jpg)<br>\* Option 2.<br><br>\* L4971D Step-Down Swithcing Voltage Regulator <br><br>\* $3.7/each <br><br>\* [link to product](https://www.digikey.com/en/products/detail/stmicroelectronics/L4971D/585932)                 | \* A switching frequency up to 300KHz is achievable<br>\* DC-DC converter include pulse-by-pulse current limit<br>\* Delivers an Output of 1.5A and 3-50V(selected by external voltage divider configuration)| \* The offered SMD-type SO16W packaging presents a chip with a whole 16 contacts, which might raise the complexity of the soldering process<br>\* Requires at least 11(!) supplementary circuit elements(capacitors, resistors, inductors) for basic configuration, which is noticeably higher than the same amount within alternatives|

### Choice:
Option #1 => LM2575 Swithcing Voltage Regulator

### Rationale:
LM2575 seems to be the most confident choice for the Voltage Regulator within the current project. It was well studied in class, expecially through breadboard-based implementation examples. Considering the selected packaging type, it will be relatively easy for soldering both through the hot air reflow and traditional, iron-based heating methods. Along with this, it is currently provided within the low price range, so it becomes possible to order more parts at a time, already accounting for potential future assembly and soldering issues

## 3. Voltage Level Translator Selection

| **Solution**                                                                                                                                                                                      | **Pros**                                                                                                                                    | **Cons**                                                                                            |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| ![](TXS0102.jpg)<br>\* Option 1.<br><br>\* TXS0102DCTR Voltage Level Translator <br><br>\* $0.49/each <br><br>\* [link to product](https://www.digikey.com/en/products/detail/texas-instruments/TXS0102DCTR/1632642)                 | \* No direction control signal needed<br>\* No power-supply sequencing required: either VCCA or VCCB can be ramped first<br>\* Known and recongised for use in setting up serial communication channels of I2C and SPI formats<br> \* Noticeably small and compact, a great fit for daughterboard-type PCB-s| \* Must be soldered with care and caution due to the tiny size<br>\* Requires a HIGH input signal from the MCU in order to operate; won't let the A and B signals pass through while in "passive mode"|
